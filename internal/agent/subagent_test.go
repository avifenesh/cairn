...
func (m *mockConfigService) PatchConfig(_ context.Context, _ map[string]any) (map[string]any, error) {
...
func TestSubagentIdentityInjection(t *testing.T) {
	...
	t.Run("normal case - valid owner injected", func(t *testing.T) {
		...
		// Should contain the canonical identity section
...
// buildSystemHintForTest extracts the identity injection logic for unit testing.
// It mirrors the code in executeSubagent without needing the full subagent infrastructure.
	...
	if r.toolConfig != nil {
		...
		if err != nil {
			...
			if isValidGitHubOwner(owner) {
				systemHint += "\n\n## Canonical Identity\n- GitHub repo owner: " + owner + " (exact spelling — never guess or fabricate this value)"
			}
		}
	}
	return systemHint
