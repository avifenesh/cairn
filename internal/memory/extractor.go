...
// nomic-embed-text returns ~0.5-0.6 for semantically identical content
// phrased differently, so thresholds must be calibrated accordingly.
const (
	thresholdDuplicate = 0.55 // Above this = already known, skip
	thresholdUpdate    = 0.45 // Between update and duplicate = refine existing
)
